...
class Database():
    ...
    def insert_element(self, collection, element) -> None:
        """
        ...
        element_id = collection.insert_one(element).inserted_id
        print(f"elemento {element_id} inserido na coleção {collection}")

    # BUSCA POR ELEMENTO NO BANCO
    def search_element(db:database.Database, collection:str , element) -> None:
        query = db.collection.find_one({"Name": str(element)})
        if query:
            print(query)
        else:
